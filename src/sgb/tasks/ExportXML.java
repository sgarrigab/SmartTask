...
public class ExportXML
{
	...
	String PROGRAMA="ExportXML";

	private Context _ctx;
	private SQLiteDatabase _db;
	private Exporter _exporter;

	public ExportXML( Activity act, SQLiteDatabase db )
	{
		_ctx = act;
		_db = db;
		...
		try
		{
			if(!Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())){
			    Toast.makeText(act, "External SD card not mounted", Toast.LENGTH_LONG).show();
			}

			// create a file on the sdcard to export the
			// database contents to
			File myFile = new File(Utilitats.getWorkFolder(
					act, Utilitats.IMPORT)
					.getAbsolutePath()
					+ "/" + EXPORT_FILE_NAME );
